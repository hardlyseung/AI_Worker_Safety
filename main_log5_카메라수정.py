...
def check_camera_device():
    if not os.path.exists("/dev/video0"):
        print("/dev/video0 장치가 존재하지 않습니다. 카메라 연결 확인 필요.")
        return False
    return True
...
def init_camera():
    ...
    if not cap.isOpened():
        ...
        return None
    print("camera is successfully connected")
    return cap
...
def main():
    ...
    try:
        while True:
            ...
            process_single_frame(resized, count)  # YOLO+Pose+LSTM 처리
            count += 1

            # 영상 출력 없이 headless로 동작 (필요 시 여기에 imshow 추가 가능)
            # 예) cv2.imshow("Live", resized)
